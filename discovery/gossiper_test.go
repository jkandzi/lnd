...
func TestSignatureAnnouncementRetryAfterBroadcast(t *testing.T) {
	...
	// Receive the remote announcement proof from the peer.
	select {
	case err = <-ctx.gossiper.ProcessRemoteAnnouncement(batch.remoteProofAnn,
		remoteKey):
	case <-time.After(2 * time.Second):
	...
	// Even though the gossiper hasn't successfully sent the local
	// announcement proof to the remote, it has received the remote one,
	// meaning that it has everything it needs to broadcast the
	// ChannelAnnouncement and ChannelUpdates to the network. Check that
	// this is done.
	for i := 0; i < 3; i++ {
		select {
		case <-ctx.broadcastedMessage:
	...
	// Even though the channel announcements are broadcasted, we expect
	// the gossiper to retry sending the AnnounceSignatures to the peer
	// when it comes back online. This is important for the remote peer,
	// since it cannot be sure if we already have broadcast the
	// announcement, so given both proofs, it can reconstruct and
	// broadcast them by itself.
	...
	// After starting up, the gossiper will see that it has a waitingproof
	// in the database, and will retry sending its part to the remote. Since
	// SendToPeer will gain fail, it should register for a notification when
