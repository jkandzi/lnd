...
func createTestFundingManager(t *testing.T, pubKey *btcec.PublicKey, tempTestDir string, hdSeed []byte,
	...
	miningNode *rpctest.Harness, chainNotifier chainntnfs.ChainNotifier,
...
func setupFundingManagers(t *testing.T) {
	...
	aliceFundingMgr, err = createTestFundingManager(t, alicePubKey, aliceTestDir, alicePrivKeyBytes[:], netParams, rpcConfig, miningNode, chainNotifier, aliceMsgChan, aliceAnnounceChan)
	if err != nil {
		t.Fatalf("failed creating fundingManager: %v", err)
	}
	...
	bobFundingMgr, err = createTestFundingManager(t, bobPubKey, bobTestDir, bobPrivKeyBytes[:], netParams, rpcConfig, miningNode, chainNotifier, bobMsgChan, bobAnnounceChan)
	if err != nil {
		t.Fatalf("failed creating fundingManager: %v", err)
	}
