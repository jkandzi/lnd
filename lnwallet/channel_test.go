...
func TestSimpleAddSettleWorkflow(t *testing.T) {
	...
	// Next alice commits this change by sending a signature message. Since we
	// expect the messages to be ordered, Bob will receive the HTLC we just sent
	// before he receives this signature, so the signature will cover the HTLC.
	aliceSig, err := aliceChannel.SignNextCommitment()
	if err != nil {
		t.Fatalf("alice unable to sign commitment: %v", err)
	}

	// Bob receives this signature message, and checks that this covers the state
	// he has in his remote log. This includes the HTLC just sent from Alice.
	if err := bobChannel.ReceiveNewCommitment(aliceSig); err != nil {
		t.Fatalf("bob unable to process alice's new commitment: %v", err)
	}
	// Bob revokes his prior commitment given to him by Alice, since he now has a
	// valid signature for a newer commitment.
	bobRevocation, err := bobChannel.RevokeCurrentCommitment()
	if err != nil {
		t.Fatalf("unable to generate bob revocation: %v", err)
	}
	// Bob finally send a signature for Alice's commitment transaction. This
	// signature will cover the HTLC, since Bob will first send the revocation
	// just created. The revocation also acks every received HTLC up to the point
	// where Alice sent here signature.
	...
	// Alice then processes bob's signature, and since she just received the
	// revocation, she expect this signature to cover everything up to the point
	// where she sent her signature, including the HTLC.
...
func TestUpdateFeeSenderCommits(t *testing.T) {
	...
	// Bob receives this signature message, and verifies that it is consistent
	// with the state he had for Alice, including the received HTLC and fee update.
	if err := bobChannel.ReceiveNewCommitment(aliceSig); err != nil {
		t.Fatalf("bob unable to process alice's new commitment: %v", err)
	}
	...
	// Bob can revoke the prior commitment he had. This should lock in the fee
	// update for him.
	bobRevocation, err := bobChannel.RevokeCurrentCommitment()
	if err != nil {
		t.Fatalf("unable to generate bob revocation: %v", err)
	...
	// Bob commits to all updates he has received from Alice. This includes the
	// HTLC he received, and the fee update.
	bobSig, err := bobChannel.SignNextCommitment()
	if err != nil {
		t.Fatalf("bob unable to sign alice's commitment: %v", err)
	}
	...
	// Alice receives new signature from Bob, and assumes this covers the changes.
	if err := aliceChannel.ReceiveNewCommitment(bobSig); err != nil {
		t.Fatalf("alice unable to process bob's new commitment: %v", err)
	}
	...
	// Alice can revoke the old commitment, which will lock in the fee update.
	aliceRevocation, err := aliceChannel.RevokeCurrentCommitment()
	if err != nil {
		t.Fatalf("unable to revoke alice channel: %v", err)
	}
...
func TestUpdateFeeReceiverCommits(t *testing.T) {
	...
	// Bob commits to every change he has sent since last time (none). He does not
	// commit to the received HTLC and fee update, since Alice cannot know if he
	// has received them.
	bobSig, err := bobChannel.SignNextCommitment()
	if err != nil {
		t.Fatalf("alice unable to sign commitment: %v", err)
	...
	// Alice receives this signature message, and verifies that it is consistent
	// with the remote state, not including any of the updates.
	if err := aliceChannel.ReceiveNewCommitment(bobSig); err != nil {
		t.Fatalf("bob unable to process alice's new commitment: %v", err)
	}

	// Alice can revoke the prior commitment she had, this will ack everything
	// received before last commitment signature, but in this case that is nothing.
	aliceRevocation, err := aliceChannel.RevokeCurrentCommitment()
	if err != nil {
		t.Fatalf("unable to generate bob revocation: %v", err)
	}
	...
	// Alice will sign next commitment. Since she sent the revocation, she also
	// ack'ed everything received, but in this case this is nothing. Since she
	// sent the two updates, this signature will cover those two.
	aliceSig, err := aliceChannel.SignNextCommitment()
	if err != nil {
		t.Fatalf("bob unable to sign alice's commitment: %v", err)
	}

	// Bob gets the signature for the new commitment from Alice. He assumes this
	// covers everything received from alice, including the two updates.
	if err := bobChannel.ReceiveNewCommitment(aliceSig); err != nil {
		t.Fatalf("alice unable to process bob's new commitment: %v", err)
	}
	...
	// Bob can revoke the old commitment. This will ack what he has received,
	// including the HTLC and fee update. This will lock in the fee update for bob.
	bobRevocation, err := bobChannel.RevokeCurrentCommitment()
	if err != nil {
		t.Fatalf("unable to revoke alice channel: %v", err)
	}
	...
	// Bob will send a new signature, which will cover what he just acked: the
	// HTLC and fee update.
	bobSig, err = bobChannel.SignNextCommitment()
	if err != nil {
		t.Fatalf("alice unable to sign commitment: %v", err)
	...
	// Alice receives revokation from Bob, and can now be sure that Bob received
	// the two updates, and they are considered locked in.
	if _, err := aliceChannel.ReceiveRevocation(bobRevocation); err != nil {
		t.Fatalf("bob unable to process alive's revocation: %v", err)
	}

	// Alice will receive the signature from Bob, which will cover what was just
	// acked by his revocation.
	if err := aliceChannel.ReceiveNewCommitment(bobSig); err != nil {
		t.Fatalf("alice unable to process bob's new commitment: %v", err)
	}
	...
	// After Alice now revokes her old commitment, the fee update should lock in
	aliceRevocation, err = aliceChannel.RevokeCurrentCommitment()
	if err != nil {
		t.Fatalf("unable to generate bob revocation: %v", err)
	}
...
func TestUpdateFeeReceiverSendsUpdate(t *testing.T) {
	...
	// Since Alice is the channel initiator, she should fail when receiving fee
	// update
	fee := btcutil.Amount(111)
...
func TestUpdateFeeMultipleUpdates(t *testing.T) {
	...
	// Alice sending more fee updates now should not mess up the old fee they
	// both committed to.
