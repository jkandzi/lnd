...
type LightningChannel struct {
	...
	// pendingAckFeeUpdate is set to the last committed fee update which is not
	// yet ACKed. Set to nil if no such update.
...
func (lc *LightningChannel) fetchCommitmentView(remoteChain bool,
	...
	if lc.channelState.IsInitiator {
		...
		// The case where we sent an update_fee message since our last commitment,
		// and now we are signing that one.
		if remoteChain && lc.pendingFeeUpdate != nil {
			feePerKw = *lc.pendingFeeUpdate
		}

		// The case where we committed to a sent fee update, and now we got a
		// commitment that ACKed that update.
		if !remoteChain && lc.pendingAckFeeUpdate != nil {
			feePerKw = *lc.pendingAckFeeUpdate
		}
	} else {

		// We received a fee update since last received commitment, so this received
		// commitment will sign that update.
		if !remoteChain && lc.pendingFeeUpdate != nil {
			feePerKw = *lc.pendingFeeUpdate
		}

		// Earlier we received a commitment that signed an earlier fee update, and
		// now we must ACK that update.
		if remoteChain && lc.pendingAckFeeUpdate != nil {
			feePerKw = *lc.pendingAckFeeUpdate
		}
	}
...
func (lc *LightningChannel) SignNextCommitment() ([]byte, error) {
	...
	// If we are the channel initiator then we would have signed any sent fee
	// update at this point, so mark this update as pending ACK, and set
	// pendingFeeUpdate to nil. We can do this since we know we won't sign any
	// new commitment before receiving a revoke_and_ack, because of the
...
func (lc *LightningChannel) ReceiveNewCommitment(rawSig []byte) error {
	...
	// If we are not channel initiator, then the commitment just received would've
	// signed any received fee update since last commitment. Mark any such fee
	// update as pending ACK (so we remember to ACK it on our next commitment),
	// and set pendingFeeUpdate to nil. We can do this since we won't receive any
	// new commitment before ACKing.
	if !lc.channelState.IsInitiator {
		lc.pendingAckFeeUpdate = lc.pendingFeeUpdate
		lc.pendingFeeUpdate = nil
	}
