...
func (s *Switch) addLink(link ChannelLink) error {
	...
	// TODO(halseth): remove this after roasbeef's commits.
	go func() {
		log.Infof("Added channel link with chan_id=%v, "+
			"short_chan_id=(%v), bandwidth=%v", link.ChanID(),
			spew.Sdump(link.ShortChanID()), link.Bandwidth())
	}()
