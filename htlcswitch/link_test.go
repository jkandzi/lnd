...
func TestChannelLinkBandwidthConsistency(t *testing.T) {
	...
	aliceLink, cleanUp, err := newSingleLinkTestHarness(chanAmt, 0)
	if err != nil {
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	aliceLink, cleanUp, err := newSingleLinkTestHarness(chanAmt, chanReserve)
	if err != nil {
		t.Fatalf("unable to create link: %v", err)
	}
	defer cleanUp()
	...
	bobLink, bobCleanUp, err := newSingleLinkTestHarness(bobChanAmt,
		bobChanReserve)
	if err != nil {
		t.Fatalf("unable to create link: %v", err)
	}
